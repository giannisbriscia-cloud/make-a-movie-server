...
def merge():
    try:
        ...
        if soundtrack:
            ...
            fs = max(0, dur - fo)
            subprocess.run(["ffmpeg","-y","-i",mp,"-ss",str(ss),"-i",sp,"-filter_complex","[1:a]atrim=start=" + str(ss) + ",asetpts=PTS-STARTPTS,afade=t=out:st=" + str(fs) + ":d=" + str(fo) + "[sa];[0:a][sa]amix=inputs=2:duration=first[aout]","-map","0:v","-map","[aout]","-c:v","copy","-c:a","aac","-shortest",op], check=True, capture_output=True)
        else:
            op = mp
